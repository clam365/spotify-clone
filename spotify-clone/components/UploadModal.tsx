...
const UploadModal = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (values) => {
      try {
          ...
          //Extracting song and image files
          ...
          //Now putting it into songs table/database
          const {error: supabaseError} = await supabaseClient.from('songs').insert({
              user_id: user.id, title: values.title, author: values.author, image_path: imageData.path, song_path: songData.path
          });
          ...
          >
              ...
              <div>
                  ...
                  <Input id="song" type="file" disabled={isLoading}
                         accept={".mp3"} {...register('song', {required: true})} />
              </div>
              <div>
                  <div className={"pb-1"}>
                      Select an image
